...
for ((i = 0; i < ${n_doms}; i++)); do
  ...
  read domain_name domain_ip_type domain_ipv4 domain_ipv6 domain_proxied domain_ttl < <(get_domain_settings "$i")
  ...
  if [ ${notifications_telegram_enabled} == true ]; then
    telegram_notification=$(
      curl -s -X GET "https://api.telegram.org/bot${notifications_telegram_bot_token}/sendMessage?chat_id=${notifications_telegram_chat_id}" --data-urlencode "text=${record} DNS record updated to: ${ip4} / ${ip6}"
    )
