...
function teardown() {
    if test -f "cloudflare-dns.log"; then
...
# bats test_tags=files
@test "Settings file validation Failure \ a" {

    # Assert
    source cloudflare-dns.sh

    # Act
    run settings_file_validation
...
# bats test_tags=files
@test "Settings file validation Failure \ b" {

    # Assert
    source cloudflare-dns.sh
    parent_path=null_path
...
# bats test_tags=files
@test "Settings file validation Failure \ c" {

    # Assert
    source cloudflare-dns.sh

    # Act
    run settings_file_validation wrong_file.yaml
...
# bats test_tags=parser
@test "Settings domains validation" {
    ...
    domains__name[2]='other.example.com'
    domains__ip_type[0]='external'
    domains__ip_type[1]='wrong'
    domains__ip_type[2]='internal'
    domains__ipv4[0]='true'
    domains__ipv4[1]='true'
    domains__ipv4[2]='wrong'
    domains__ipv6[0]='true'
    domains__ipv6[1]='true'
    domains__ipv6[2]='wrong'
    domains__proxied[0]='true'
    domains__proxied[1]='wrong'
    domains__proxied[2]='true'
    domains__ttl[0]='wrong'
    domains__ttl[1]='3600'
    domains__ttl[2]='wrong'


    # Stub
    function get_n_items() { echo "3";}
    export -f get_n_items
...
# bats test_tags=parser
@test "Settings API validation failure \ a" {

    # Arrange
    source cloudflare-dns.sh
    settings_cloudflare__zone_id=""
    settings_cloudflare__zone_api_token="ok"

    # Act
    run settings_validation

    # Assert
    assert_line --partial "Error! Cloudflare Zone ID not set"

}

# bats test_tags=parser
@test "Settings API validation failure \ b" {

    # Arrange
    source cloudflare-dns.sh
    settings_cloudflare__zone_id="ok"
    settings_cloudflare__zone_api_token=""

    # Act
    run settings_validation

    # Assert
    assert_line --partial "Error! Cloudflare API Token not set"
...
# bats test_tags=get_ip
@test "External Validation failure" {

    # Arrange
    source cloudflare-dns.sh

    # Act
    run external_validation
...
# bats test_tags=get_ip
@test "Internal Validation failure" {

    # Arrange
    source cloudflare-dns.sh

    # Act
    run internal_validation
...
# bats test_tags=process_domains
@test "process_domains  \ a" {
    ...
    domains__ttl[2]='auto'
    api_zone_id='<your_id>'
    api_zone_token='<your_token>'
    notifications_telegram_enabled=false
    ip4="1.2.3.4"
    ip6="1111:2222:3333:4444:5555:6666:7777:8888"
    error_dom=false
    is_proxied4=true
    is_proxied6=true
    dns_record_ip4="1.2.3.4"
    dns_record_ip6="1111:2222:3333:4444:5555:6666:7777:8888"

    # Stub functions
    function read_record() { echo "${api_read_response_ip4}"; }
    function api_validation() { return 0;}
    function get_ip() { return 0;}
    function get_dns_record_ip() { return 0;}
    function push_validation() { return 0;}
    export -f read_record
    export -f api_validation
    export -f get_ip
    export -f push_validation
    ...
    unset -f api_validation
    unset -f get_ip
    unset -f push_validation
    unset -f get_dns_record_ip
}

@test "process_domains  \ b" {
...
# bats test_tags=notifications
@test "Telegram notifications Success" {
    ...
    # Stub functions
    function api_telegram() { echo "\"ok\":true"; }
    export -f api_telegram

    # Act
    telegram_notification

    # Clean up
    unset -f api_telegram
}

# bats test_tags=notifications
@test "Telegram notifications Failure" {
    ...
    # Stub functions
    function api_telegram() { echo "\"ok\":false"; }
    export -f api_telegram

    # Act
    run telegram_notification

    # Assert
    assert_line --index 1 "Error! Telegram notification failed"
