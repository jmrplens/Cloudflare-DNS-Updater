...
Pre-compiled binaries bundle Bash and jq, the two dependencies a host is most likely to lack or to have in too old a version. `curl` and the usual command line tools are taken from the system, and the self-extracting launcher needs `tar` (PowerShell on Windows) to unpack itself.
