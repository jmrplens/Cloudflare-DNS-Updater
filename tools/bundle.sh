...
{
	echo '#!/bin/bash'
	echo '# Monolith build generated by tools/bundle.sh'
} >"$OUTPUT"
...
for file in src/logger.sh src/network.sh src/config.sh src/ip.sh src/cloudflare.sh src/notifications.sh; do
	...
	{
		echo -e "\n# --- Module: $file ---"
		sed '/^#!/d; /^[[:space:]]*source /d; /^[[:space:]]*# shellcheck /d' "$file"
	} >>"$OUTPUT"
done
...
{
	echo -e "\n# --- Module: src/main.sh ---"
	sed '/^#!/d; /^[[:space:]]*source /d; /^[[:space:]]*# shellcheck /d; /^[[:space:]]*DIR=/d; /main \"\$@\"$/d' src/main.sh
} >>"$OUTPUT"
